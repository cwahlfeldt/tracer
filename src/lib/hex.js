...
export const LAYOUT = layout(point(SIZE, SIZE), point(ORIGIN, ORIGIN))

export function hex(q, r, s) {
    if (q + r + s !== 0) throwError('q + r + s must equal 0')
...
export function fracHexagon(q, r, s) {
    if (round(q + r + s) !== 0) throwError('q + r + s must equal 0')
...
export function multiplyHex(h, multiplyBy) {
    return hex(h.q * multiplyBy, h.r * multiplyBy, h.s * multiplyBy)
}
...
export function convertHexToPixel(hex) {
    ...
    return point(
        Math.floor(x + LAYOUT.origin.x),
        Math.floor(y + LAYOUT.origin.y)
    )
}
...
export function convertPixelToHex(pixelPoint) {
    ...
    return hex(roundedHexCoords.q, roundedHexCoords.r, roundedHexCoords.s)
}
...
export function hexLerp(a, b, t) {
    return roundHex(
        fracHexagon(lerp(a.q, b.q, t), lerp(a.r, b.r, t), lerp(a.s, b.s, t))
    )
}
...
export function shuffleGrid(grid) {
    ...
    for (let i = 0; i < size; i++)
        newGrid.splice(randomInt(0, grid.length - 1), 1)
